...
interface IFlowConfigSelectorProps {
...
function ConfigOptions({
  ...
  return (
    <div className={variant === "mobile" ? "mt-6 space-y-5" : "space-y-4 p-3"}>
      {/* Model Selection */}
          ...
          {MODELS.map((model) => (
            <button
              ...
              className={`w-full text-left px-3 py-2.5 rounded-md transition-all ${
                variant === "mobile" ? "touch-manipulation" : ""
              } ${
      ...
      {variant === "mobile" && onClose && (
        <Button onClick={onClose} className="w-full h-10 touch-manipulation mt-6">
          Done
        </Button>
      )}
    </div>
...
export function IFlowConfigSelector({
  modelName,
  permissionMode,
  onModelChange,
  onPermissionChange,
  ...
  const triggerButton = (
    ...
    >
      <span className="font-medium">
        {getShortLabel(modelName, permissionMode)}
      </span>
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="w-[360px] max-h-[600px] overflow-y-auto p-0"
        align="start"
      >
